...
const SingleBookingSearch = () => {
  ...
  useEffect(() => {
    const fetchBooking = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/bookings/lr/${encodeURIComponent(lrNo)}`);
        if (res.ok) {
          const data = await res.json();
          setBooking(data);
        } else {
  ...
  const printLrCopy = () => {
    ...
    const url = `http://localhost:5000/api/bookings/lr/${encodeURIComponent(booking.lrNo)}/pdf`;
